...
export class Invoice {
  ...
  constructor(options) {
    ...
    this._options.paymentMethod =
      options.paymentMethod || defaultOptions.paymentMethod;
    ...
    this._options.exchangeRate =
      options.exchangeRate || defaultOptions.exchangeRate;
    this._options.exchangeBank =
      options.exchangeBank || defaultOptions.exchangeBank;
  ...
  _generateXML(indentLevel = 0) {
    assert(
      this._options.issueDate instanceof Date,
      'Valid IssueDate field missing from invoice options',
    );

    assert(
      this._options.fulfillmentDate instanceof Date,
      'Valid FulfillmentDate field missing from invoice options',
    );

    assert(
      this._options.dueDate instanceof Date,
      'Valid DueDate field missing from invoice options',
    );

    assert(
      this._options.paymentMethod instanceof PaymentMethod,
      'Valid PaymentMethod field missing from invoice options',
    );

    assert(
      this._options.currency instanceof Currency,
      'Valid Currency field missing from invoice options',
    );

    assert(
      this._options.language instanceof Language,
      'Valid Language field missing from invoice options',
    );

    assert(
      typeof this._options.seller === 'undefined' ||
        this._options.seller instanceof Seller,
      'The provided optional Seller field is invalid',
    );

    assert(
      this._options.buyer instanceof Buyer,
    ...
    o += this._options.items
      .map((item) => {
        assert(
          ...
          'Element in Items array is not an instance of the Item class',
        );
